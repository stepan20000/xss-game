...
<html>
 ...
 <!-- DOMPurify script  -->
 <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/dompurify/1.0.4/purify.js"></script>
  </head>
...
class MainPage(webapp.RequestHandler):
  ...
  def get(self):
    ...
    if not self.request.get('query'):
      ...
      safeQuery = sanitize_html(query)

      # Our search engine broke, we found no results :-(
      message = "Sorry, no results were found for <b><script>document.write(DOMPurify.sanitize('" + safeQuery + "'))</script></b>."
